...
In Apollo Server 4, the `debug` constructor option has been removed. In its place is a new `includeStacktraceInErrorResponses` option which controls its namesake feature. Like `debug`, this option defaults to `true` unless the `NODE_ENV` environment variable is either `production` or `test`. If you use `debug` in Apollo Server 3, you can use `includeStacktraceInErrorResponses` with the same value in Apollo Server 4:
...
### Built-in error extensions

Apollo Server 3 uses the `declare module` TypeScript feature to allow you to write `error.extensions.exception.code` and `error.extensions.exception.stacktrace` where `error` is any `GraphQLError` even though `exception` is an Apollo Server-specific concept. Because not all GraphQLErrors are guaranteed to have these fields (and in fact, they are added to the `GraphQLFormattedError` rather than the `GraphQLError`, Apollo Server 4 does not use this feature; the values `error.extensions.code` and `error.extensions.stacktrace` will have type `unknown`. (Note that these fields are also no longer nested inside `exception`.
